                                          ...
                                          ui(new Ui::MainWindow)
{
    ...
    Selectbin->setIcon(QIcon(":/images/application-x-executable.svg"));
    ...
    Selectdir->setIcon(QIcon(":/images/folder.svg"));
...
{
    QString path = ui->lineEdit_exec->text().isEmpty() ? "~" : ui->lineEdit_exec->text();
    QString file = QFileDialog::getOpenFileName(this, "选择应用程序", path, "脚本文件(*.sh);;全部文件(*)");
    if (file.isEmpty())
        return;
...
void MainWindow::on_lineEdit_exec_textChanged(const QString &arg1)
{
    ...
    QString name;
    if (end < start)
        path.mid(start, path.length() - start);
    else
        path.mid(start, end - start);
...
void MainWindow::on_toolButton_2_clicked()
{
    ...
    if (!desktopfile.isWritable())
        mbc.exec(); //不可写的情况下，没必要更改
